...
public class AutoSell implements IFeature {
    ...
    public void stop() {
        ...
        if (MacroHandler.getInstance().isMacroToggled() && !VisitorsMacro.getInstance().isRunning() && !AutoComposter.getInstance().isRunning() && !AutoTrap.getInstance().isRunning()) {
    ...
    public void onTickEnabled(TickEvent.ClientTickEvent event) {
        ...
        if (FeatureManager.getInstance().isAnyOtherFeatureEnabled(this, VisitorsMacro.getInstance(), AutoComposter.getInstance(), AutoTrap.getInstance())) return;
